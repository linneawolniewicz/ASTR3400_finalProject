...
## -----------------------------------
    ...
    ####### parameters ###################
        ...
        self.r_outer     = 1e15 # initial outer boundary radius
    ...
    def setup_initial_conditions(self):
        ...
        elif (self.itype==self.HEGER) | (self.itype==self.MESA):
            if self.itype==self.HEGER:
                ...
                print('Read data using MESA.')
                m=mesa.mesa(path=self.mpath,name=self.mname)
                m.read_mesa_star()
                if self.hent > 0:
                    s4, = np.where(m.data['entropy'] > self.hent)
                    self.r_inner = m.data['radius'][s4[-1]]*7e10
                    self.r_outer = m.data['radius'][s4[0]]*7e10
                    print('outer radius: ',self.r_outer)
                    print('Inner radius: ',self.r_inner)
                    indx = np.arange(len(m.data['radius']))[(m.data['radius']*7e10 > self.r_inner)&(m.data['radius']*7e10 < self.r_outer)]
                    rvals=m.data['radius'][indx][::-1]*7e10
                    pvals=10**m.data['logP'][indx][::-1]
                    dvals=10**m.data['logRho'][indx][::-1]
                    vvals=m.data['velocity'][indx][::-1]

                # set inner mass to mass coordinate of r_inner:
                self.mass_r_inner=m.data['mass'][indx][-1]*pc.m_sun
